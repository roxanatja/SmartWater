...
import { generalTemplate } from '../pdfTemplates';

interface Props {
    allComissions: Comission<'general'>[];
    selectedInventario: Comission<'general'>;
    distribuidores: User[];
    onCancel: () => void
}

const ShowComisionesGenerales = ({ allComissions, selectedInventario, distribuidores, onCancel }: Props) => {
    ...
    const report = async () => {
        ...
        if (array.length > 0) {
            ...
            showGeneratePDF(setLoading, generalTemplate, inputs)
        }
    }
...
export default ShowComisionesGenerales