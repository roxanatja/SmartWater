...
const Prestamos: FC = () => {
    ...
    if (error) {
        return <p>Ocurrio un error en la carga de datos, intentelo nuevamente en unos minutos.</p>;
    };
                ...
                <FiltroPaginado filtro resultadosPrestamo onFilter={Onfilter} total={loans.length}>
                    <div style={{display: "flex", flexWrap: "wrap", gap: "30px"}}>
                        {loans.map((loan, index) => {
                            ...
                            return <CuadroPrestamo key={index} loan={loan} productos={products} estadoContrato={contratcEstate}/>
                        })}
                    </div>
                </FiltroPaginado>
            </div>
