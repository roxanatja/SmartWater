...
const EgresosGastos: FC = () => {
        ...
        reports.forEach(r => {
            ...
            if (idx !== -1) {
                ...
                if (dateIdx !== -1) {
                    ...
                    aux[idx].sales[dateIdx] = { amount: cpy.amount + r.amountStatic || r.amount, date: cpy.date }
                } else {
                    aux[idx].sales.push({ amount: r.amountStatic || r.amount, date: date.format("YYYY-MM-DD") })
                }
            } else {
                aux.push({
                    product: r.accountEntry._id,
                    sales: [{
                        amount: r.amountStatic || r.amount,
    ...
    const loadData = useCallback(async () => {
        const res = await ExpensesApiConector.get({ pagination: { page: 1, pageSize: 30000 }, filters: { initialDate: filters.initialDate || "2020-01-01", finalDate: filters.finalDate || moment().format("YYYY-MM-DD") } })
        setReports(res?.data || [])
    }, [filters])
