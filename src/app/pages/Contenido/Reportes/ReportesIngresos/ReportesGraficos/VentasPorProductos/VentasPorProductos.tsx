...
const VentasPorProductos: FC = () => {
    ...
    const dataPie = useMemo<ChartData<'bar', number[], string>>(() => {
        return {
            ...
            datasets: [{
                data: productsSelected.map(p => (formatted.find(f => f.product === p.name)?.sales || []).reduce((acc, curr) => acc += curr.amount, 0)),
    ...
    const total = useMemo<number>(() => {
        return formatted
            .filter(f => productsSelected.some(p => p.name === f.product))
